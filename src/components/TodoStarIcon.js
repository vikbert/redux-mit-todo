...
const TodoStarIcon = (props) => {
  const {todo, counterActiveStarred, updateTodo, toggleItemStarState} = props;
  const [starred, setStarred] = useState(todo.starred);
  const iconClass = starred ? (todo.completed ? 'starred starred_completed' : 'starred') : 'star';

  const onClickHandler = () => {
    if (counterActiveStarred === 3) {
    ...
    newTodo.starred = +!todo.starred;
    updateTodo(newTodo, todo);
    toggleItemStarState(newTodo.starred);

    setStarred(newTodo.starred);
  };
